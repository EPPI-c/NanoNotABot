...
class Rank(commands.Cog):
    ...
    async def his(self, ctx, amount:int):
        async with ctx.typing():
            ...
            self.ranking[ctx.channel.id] = [m async for m in messages if m.content.startswith('>>')]

    @commands.command(name='>')
    async def entry(self, ctx):
        async with ctx.typing():
            if channel := self.ranking.get(ctx.channel.id):
                channel.append(ctx.message)
            else:
                self.ranking[ctx.channel.id] = [ctx.message]
            for e in onetoten:
